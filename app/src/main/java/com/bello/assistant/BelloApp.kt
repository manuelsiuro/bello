...
class BelloApp : Application() {
    ...
    private fun scheduleRestart() {
        // A crash at startup would otherwise restart into itself for ever.
        val aliveMs = SystemClock.elapsedRealtime() - startedAt
        val prefs = getSharedPreferences("bello", Context.MODE_PRIVATE)
        val consecutive = prefs.getInt(CRASH_COUNT, 0)
        val delay = RestartBackoff.delayMs(consecutive, aliveMs)
        prefs.edit().putInt(CRASH_COUNT, RestartBackoff.nextCount(consecutive, aliveMs)).commit()
        ...
        (getSystemService(Context.ALARM_SERVICE) as AlarmManager)
            .set(AlarmManager.ELAPSED_REALTIME_WAKEUP, SystemClock.elapsedRealtime() + delay, pending)
        FileLog.w("crash", "restart scheduled in ${delay}ms (crash ${consecutive + 1} in a row, " +
            "process lived ${aliveMs / 1000}s)")
    }

    private companion object { const val CRASH_COUNT = "crashesInARow" }
}
