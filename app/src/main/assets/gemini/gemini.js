...
// Selectors are isolated here so they can be updated without rebuilding the app (FR-GWEB-04):
// push a new version to <app files>/gemini.js and it is used instead of this one.
// Talks to Kotlin through the BelloWeb bridge (see llm/GeminiWebProvider.kt).
...
(function (b) {
  ...
  function log(m) { try { BelloWeb.log(String(m)); } catch (e) {} }
  function done(id, obj) { try { BelloWeb.result(id, JSON.stringify(obj)); } catch (e) {} }
  ...
  // True when the page can take a question right now (startup health check, FR-GWEB-06).
  b.ready = function () {
    return !b.blocked() && !!document.querySelector(SEL.editor);
  };
